...
export default async function UpgradePage({ searchParams }: { searchParams: Promise<{ reason?: string; used?: string; error?: string; success?: string }> }) {
  ...
  const percentage = limit === null ? 0 : Math.min(Math.round((used / limit) * 100), 100);
  const reachedLimit = params.reason === 'order-limit' || usage.isAtLimit;
  const activePaidPlan = !isFreePlan && business.plan_expires_at && new Date(business.plan_expires_at) > new Date();
  const expiryDate = business.plan_expires_at ? new Date(business.plan_expires_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'long', year: 'numeric' }) : null;
    ...
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      <div className="card overflow-hidden border-amber-200 bg-gradient-to-br from-amber-50 via-white to-emerald-50 p-6 sm:p-10">
        <span className="inline-flex rounded-full bg-amber-100 px-3 py-1 text-xs font-bold uppercase tracking-wide text-amber-800">
          {isFreePlan ? 'Free plan' : `${business.plan} plan`}
        </span>
        ...
        <p className="mt-3 max-w-2xl text-slate-600">
          {reachedLimit
            ...
            : activePaidPlan
              ? `Your ${business.plan} plan is active until ${expiryDate}.`
              : `You have used ${used} of ${FREE_ORDER_LIMIT} orders this month. Upgrade whenever you need more room.`}
        </p>
        ...
        {isBusinessPlan ? (
          <div className="mt-8 rounded-2xl bg-white/80 p-5 shadow-sm ring-1 ring-slate-200">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-medium text-slate-500">Business plan status</p>
                <p className="mt-1 text-2xl font-black text-slate-900">✓ Active</p>
                <p className="mt-1 text-sm text-slate-500">Unlimited orders while your plan is active.</p>
              </div>
              {expiryDate && (
            ...
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-3xl font-black text-slate-900">
                  {used} <span className="text-base font-medium text-slate-500">/ {isProPlan ? PRO_ORDER_LIMIT : FREE_ORDER_LIMIT}</span>
                </p>
              </div>
