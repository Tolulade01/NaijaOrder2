      ...
      <section className="mx-auto max-w-6xl px-6 py-20 sm:py-24">
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-3">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative flex flex-col rounded-[2rem] bg-white p-7 shadow-sm sm:p-8 ${
                plan.featured ? 'border-2 border-emerald-900 shadow-xl' : 'border border-emerald-950/10'
              }`}
            >
              {plan.featured && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-amber-400 px-4 py-1.5 text-xs font-black text-emerald-950">
                  {plan.badge}
                </div>
              )}
              ...
              <a
                href={plan.href}
                className={`mt-9 block rounded-2xl px-6 py-4 text-center font-bold transition ${
                  plan.featured
                    ? 'bg-emerald-900 text-white hover:bg-emerald-800'
                    : 'border border-emerald-900 text-emerald-950 hover:bg-emerald-50'
                }`}
              >
                {plan.cta}
              </a>
        ...
        <p className="mx-auto mt-8 max-w-2xl text-center text-sm leading-6 text-slate-500">
          Paid-plan billing will be activated before paid plans are enforced. We&apos;ll clearly show the plan, price and payment terms before charging your account.
        </p>
      </section>
    </main>
  );
